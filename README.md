...
- `객실 전체 조회`로 전국의 객실을 모두 조회할 수 있어요
- `테마별 숙소 조회`로 호캉스부터 저렴한 게스트 하우스까지!
- 오늘 어디가지? `인기 숙소 패키지 부터 투어까지` 메인배너에서 확인하세요!
- 난 잘 모르겠어,, 그냥 제주에 있는 숙소 다 보고싶어 `국내 여행지`에 지역명을 눌러보세요!
- 이 숙소 괜찮은데 근처에 뭐가 있지? `검색 된 숙소를 지도와 함께 볼 수 있어요`
- 회원가입 시 `유효성 검사(Form validation)` 처리

<img width="100%" alt="join" src="https://github.com/Jae-Won-Song/MINI_FE/assets/120430844/1549ce00-d240-4daa-b1f8-7134d3a85a14">

<hr/>

### 메인

- `지역` `날짜` `인원` 으로 검색 가능한 검색기능
- `지금 핫한 곳이 어디지?` 인기 있는 국내 여행 패키지!
- `난 호캉스 가고싶어~~` 테마별 숙소 한눈에 확인!
- 국내 갈 만한 곳이 어디있지? `국내 여행지` 카테고리에서 확인하세요!
